	   ...
	   <div style="margin-left:10%; margin-right:10%; margin-top:50px; margin_bottom:50px; text-align:center; background:#000066; color:#99ffff ">
	        <?php
                 ...
                 require_once('Connect.php');
                    ...
                    if(isset($_POST['id']) && isset($_POST['fname']) && isset($_POST['lname']) && isset($_POST['address']) && isset($_POST['nominee']) )
					{
	                   $u = $_POST['id'];
	                   $p = $_POST['fname'];
					   $t = $_POST['lname'];
					   $v = $_POST['address'];
					   $n = $_POST['nominee'];
					   $l= "UPDATE account SET nominee= '$n', fname = '$p', lname = '$t', address = '$v' WHERE id = '$u'";
					   $result = mysqli_query($conn, $l);
					   if(mysqli_affected_rows($conn)){
			?>
			<div class="row" style="text-align: center; font-size: 100px; background:#4d0039;">
					 <a href="home.php" style="color:#1bf21b; background:#99ffff"> Update Successful </a> </div></br> <br/>
			<?php
			}
			else{
				?>
			<div class="row" style="text-align: center; font-size: 100px; background:#4d0039;">
					 <a href="home.php" style="color:#ff0000; background:#99ffff"> Update Unsuccessful </a> </div></br> <br/>
			<?php
			     }
					}
			?>
